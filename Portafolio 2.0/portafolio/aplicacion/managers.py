...
class AgregarReservaManager(models.Manager):
    ...
    def total_cobrar(self):
        ...
        consulta = self.aggregate(
            total=Sum(
                F('count')*F('departamento__Valor_Diario'),
...
class ReservaDetalleManager(models.Manager):
    ...
    def restablecer_stok_num_ventas(self, id_venta):
        ...
        for venta_detail in self.filter(venta__id=id_venta):
            ...
            venta_detail.departamento.habitaciones = venta_detail.departamento.habitaciones + venta_detail.count
            venta_detail.departamento.Valor_Diario = venta_detail.departamento.Valor_Diario - venta_detail.count
            prods_en_anulados.append(venta_detail.departamento)
        Departamento.objects.bulk_update(prods_en_anulados, ['habitaciones', 'Valor_Diario'])
        return True
